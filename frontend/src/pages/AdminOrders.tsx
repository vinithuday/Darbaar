            ...
            <div style={{ display: "flex", gap: 12, marginTop: 12, flexWrap: "wrap" }}>
              <label style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 14, color: "#374151" }}>
                ...
                <input
                  ...
                  onChange={(e) => setStartIn((m) => ({ ...m, [o._id]: Number(e.target.value) }))}
...
function OrderSection({ title, count, children }: { title: string; count: number; children: React.ReactNode }) {
  return (
    <section style={{ border: "1px solid #e5e7eb", borderRadius: 14, padding: 16, marginBottom: 20, background: "#fff" }}>
...
function OrderCard({ order, right, children }: { order: AdminOrder; right?: React.ReactNode; children?: React.ReactNode }) {
  return (
    <div style={{ border: "1px solid #e5e7eb", borderRadius: 14, padding: 14, marginBottom: 12, background: "#fafafa" }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 16, alignItems: "center", marginBottom: 8 }}>
        <div style={{ fontWeight: 700 }}>
          #{niceId(order)} {dot}
          <span style={{ color: "#6b7280", fontWeight: 400 }}>{statusBadge(order.status)}</span>
        </div>
...
function Stepper({ status }: { status: Status }) {
  ...
  return (
    <div style={{ marginTop: 12 }}>
      <div style={{ position: "relative", height: 8, background: "#e5e7eb", borderRadius: 999 }}>
        <div
          style={{
            position: "absolute",
            left: 0,
      ...
      >
        <div style={{ textAlign: "center", fontWeight: idx >= 0 ? 700 : 400 }}>Order accepted</div>
        <div style={{ textAlign: "center", fontWeight: idx >= 1 ? 700 : 400 }}>Being prepared</div>
        <div style={{ textAlign: "center", fontWeight: idx >= 2 ? 700 : 400 }}>Ready to collect</div>
        <div style={{ textAlign: "center", fontWeight: idx >= 3 ? 700 : 400 }}>Collected</div>
      </div>
    </div>
  );
}
